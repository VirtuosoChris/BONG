...
INT_PTR  CALLBACK DlgProcObject(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam){
	...
	switch(message){
		case WM_INITDIALOG: 
			SetDlgItemText(hDlg,IDC_EDIT9, objectstring);//height}
			...
			SetDlgItemInt(hDlg, IDC_EDIT1, selectedbouncer->x, TRUE);//x
				...
				//SetDlgItemInt(hDlg, IDC_EDIT3, selectedbouncer->width, TRUE);//width
				//SetDlgItemInt(hDlg, IDC_EDIT4, selectedbouncer->height, TRUE);//height
				SetDlgItemInt(hDlg, IDC_EDIT5, selectedbouncer->round, TRUE);//round #
//				if(selectedbouncer->color == RED){CheckRadioButton(hDlg, IDC_RADIO1,IDC_RADIO2, IDC_RADIO1);}
//				else{CheckRadioButton(hDlg, IDC_RADIO1,IDC_RADIO2, IDC_RADIO2);}
				SetDlgItemInt(hDlg, IDC_EDIT6, selectedbouncer->multiplier, TRUE);//reflection vector
				//SetDlgItemInt(hDlg, IDC_EDIT7, MAPBARRIER, TRUE);//hit points
			...
			if(selectedessence){
				...
				//SetDlgItemInt(hDlg, IDC_EDIT4, selectedessence->height, TRUE);//height
				SetDlgItemInt(hDlg, IDC_EDIT5, selectedessence->round, TRUE);//round #
				//if(selectedessence->color == RED){iColor = RED;CheckRadioButton(hDlg, IDC_RADIO1,IDC_RADIO2, IDC_RADIO1);}
				//else{iColor = BLUE;CheckRadioButton(hDlg, IDC_RADIO1,IDC_RADIO2, IDC_RADIO2);}
				//SetDlgItemInt(hDlg, IDC_EDIT6, MAPBARRIER, TRUE);//reflection vector
				//SetDlgItemInt(hDlg, IDC_EDIT7, MAPBARRIER, TRUE);//hit points
			...
			if(selectedgoal){
				...
				//if(selectedgoal->color == RED){iColor = RED;CheckRadioButton(hDlg, IDC_RADIO1,IDC_RADIO2, IDC_RADIO1);}
				//else{iColor = BLUE;CheckRadioButton(hDlg, IDC_RADIO1,IDC_RADIO2, IDC_RADIO2);}
				//SetDlgItemInt(hDlg, IDC_EDIT6, MAPBARRIER, TRUE);//reflection vector
				//SetDlgItemInt(hDlg, IDC_EDIT7, MAPBARRIER, TRUE);//hit points
				//SetDlgItemInt(hDlg, IDC_EDIT8, MAPBARRIER, TRUE);//radius
			}
	...
	switch(message){
		case WM_INITDIALOG:
			...
			if(gridtoggle == true){
				...
				//!IsDlgButtonChecked(hDlg, IDC_CHECK4)
			}
...
LRESULT CALLBACK MiniMapProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam){
	...
	switch(message){
		case WM_CREATE:
			...
			//hdCh = GetDC(hwndCh);
			//SetPixelFormat(hdCh, ChoosePixelFormat(hdCh, &pfd2),&pfd2);
